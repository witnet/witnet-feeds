...
async function main () {
    ...
    requests = requests
        .map(caption => {
            ...
            const networks = rulebook.getPriceFeedNetworks(caption, witnet.network === "mainnet")
            return [
                caption,
                artifact,
                request,
    ...
    requests = await Promise.all(
        requests.map(async ([caption, artifact, request, networks]) => {
            let [ dataRequests, dryRunResult ] = await Promise.all([
                ...
                request.execDryRun(true).catch(err => console.error(`  ${caption}: Error: ${err}`))
            ])
            ...
            const updateConditions = rulebook.getPriceFeedUpdateConditions(caption, witnet.network === "mainnet")
