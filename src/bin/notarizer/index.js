...
async function main() {
    ...
    program
        ...
        .option("--min-utxos <number>", "Min. UTXOs threshold", process.env.WITNET_PFS_WIT_MIN_UTXOS || 32)
        .option("--network <evm_network>", "Focus only on price feeds deployed in this EVM network", undefined)
    ...
    let { configPath, debug, minBalance, minUtxos, network, priority, provider, signer, strategy } = program.opts()
    ...
    console.info(`Initial balance:   ${balance.toString(2)} (${ledger.cacheInfo.size} UTXOs)`)
    if (balance.pedros < minBalance.pedros) {
        ...
        console.info(
            `> Checking balance schedule: ${CHECK_BALANCE_SCHEDULE}`,
        )
        cron.schedule(CHECK_BALANCE_SCHEDULE, async () => checkWitnetBalance())
    }

    const priceFeeds = await reloadRadonRequests(network, wallet.provider.network === "mainnet")
    ...
    Object.entries(priceFeeds).forEach(([caption, { conditions }]) => {
        ...
        console.info(
            `[${caption}${" ".repeat(maxWidth - caption.length)}] Update conditions: { deviation: ${
    ...
    async function notarize(caption) {
        ...
        try {
            ...
            let tx = await DRs.sendTransaction({ witnesses: conditions.minWitnesses })
        ...
        const elapsed = Math.floor(Date.now() / 1000) - lastUpdates[caption].timestamp;
        const remaining = conditions.cooldownSecs - elapsed
        const timeout = remaining > 0 ? Math.min(remaining, DRY_RUN_POLLING_SECS) : 0
        setTimeout(() => notarize(caption), timeout * 1000)
    }

    async function checkWitnetBalance() {
        try {
            let newBalance = Witnet.Coins.fromPedros((await ledger.getBalance()).unlocked)
            const now = Math.floor(Date.now() / 1000)
            const increased = newBalance.nanowits > balance?.nanowits || 0n
            const utxos = (await ledger.getUtxos(increased)).filter(utxo => utxo.timelock <= now)
            if (increased && utxos.length < minUtxos * 2) {
                ...
                for (let ix = 0; ix < iters; ix++) {
                    ...
                    newBalance = Witnet.Coins.fromPedros((await ledger.getBalance()).unlocked)
                }
            }
            balance = newBalance
        } catch (err) {
    ...
    async function reloadRadonRequests(network, mainnets) {
        const captions = []
        const rulebook = configPath ? await Rulebook.fromUrlBase(configPath) : Rulebook.default()
        ...
        return Object.fromEntries(
            ...
            .map(caption => {
                ...
                const conditions = rulebook.getPriceFeedUpdateConditions(caption, mainnets)
