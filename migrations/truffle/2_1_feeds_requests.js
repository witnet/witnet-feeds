...
async function settlePriceFeedsRadHash(from, addresses) {
    ...
    for (const key in addresses) {
        ...
        try {
            ...
            var request = await WitnetRequest.at(addresses[key])
            if ((await web3.eth.getCode(request.address)).length > 2) {
                var requestClass = await request.class.call({ from })
                if (requestClass !== "0x70aa025b") {
                    throw `contract at ${addresses[key]} not of the WitnetRequest kind.`
                }
                var radHash = await request.radHash.call({from})
                if (!(await feeds.supportsCaption.call(caption, {from}))) {
                    ...
                    var tx = await feeds.methods['settleFeedRequest(string,bytes32)'](
                        caption,
                        radHash,
                        { from }
                    )
                    ...
                    var currentRadHash = await feeds.lookupRadHash.call(hash, {from})
