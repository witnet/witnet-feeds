...
module.exports = {
    ...
    "ticker#9/mojitoswap": retrievals.GraphQLQuery({
        url: "https://thegraph.kcc.network/subgraphs/name/mojito/swap",
        script: RadonScript(RadonString).parseJSONMap().getMap("data").getMap("pair").getFloat("token\\1\\Price").multiply(1e9).round(),
        ...defaultQuery,
    }),
    ...
    "ticker/ubeswap": retrievals.GraphQLQuery({
        ...
        script: RadonScript(RadonString).parseJSONMap().getMap("data").getArray("pairs").filter(
                RadonScript(RadonMap).getString("id").match(
                    RadonBoolean,
                    { "\\0\\": true },
                    false
                )
